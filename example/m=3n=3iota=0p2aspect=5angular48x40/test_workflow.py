...
def status():
    return dict(decision='complete',last_recorded_step=10,owned_run_processes=[],exit={'returncode':0},summary={'status':'step_budget_reached','target_step':10,'final_step':10},numerical_failure_events=[],missing_periodic_snapshots=[],metrics={'checkpoint_sha256':'verified'})
...
def test_only_verified_validation_exit_is_restartable_for_production():
    s=status();s.update(segment_count=1,active_seconds=100,wall_seconds=100,nonprogress_interruptions=0,disk_free_bytes=10000,last_event={})
    p=dict(target_step=3000,max_active_seconds=86400,max_wall_seconds=172800,max_segments=16,max_nonprogress_interruptions=3,min_free_bytes=100)
...
def test_invalid_validation_cannot_promote(bad):
    s=status()
    if bad=='wrong_count':s['last_recorded_step']=9
    if bad=='failed_exit':s['exit']['returncode']=-9
    if bad=='live_process':s['owned_run_processes']=['runner']
    if bad=='numerical_failure':s['numerical_failure_events']=[{'error':'failed'}]
    with pytest.raises(AssertionError):w.validate_ten(s)
def test_no_production_transition_in_read_only_tick(tmp_path,monkeypatch):
    monkeypatch.setattr(w,'ROOT',tmp_path);c.write(tmp_path/'policy.json',{'target_step':10})
