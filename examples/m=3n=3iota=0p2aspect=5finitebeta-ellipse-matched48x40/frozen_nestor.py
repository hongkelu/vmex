...
sys.path[:0]=[str(ROOT.parents[1]),str(ROOT),'/root/autodl-tmp/vmex-rotating-ellipse-m3n3-maxmode1-full111-20260825T141353Z/external/ESSOS']
...
fixed=json.loads((ROOT/'fixed/summary.json').read_text());fit=json.loads((ROOT/'coils/summary.json').read_text())
...
assert inp.lfreeb
raw=json.loads((ROOT/'coils/coils_candidate.json').read_text());builder=Full111FieldBuilder(jnp.asarray(raw['dofs_curves']),jnp.asarray(raw['dofs_currents']));p=jnp.zeros(111)
