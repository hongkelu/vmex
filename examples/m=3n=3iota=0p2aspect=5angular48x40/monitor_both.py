...
CASES = {
    "refined_lhk3_6_gpu0": (
        "lhk3-6",
        "/root/autodl-tmp/vmex-m3n3-iota02-aspect5-angular48x40-20260914/venv/bin/python",
        "/root/autodl-tmp/vmex-m3n3-iota02-aspect5-angular48x40-20260914/workflow.py",
    ),
}
...
def main():
    ...
    a = p.parse_args()
    out = Path(__file__).resolve().parent / "runs/high-resolution-monitor"
    out.mkdir(parents=True, exist_ok=True)
    with (out / "monitor.lock").open("a") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError:
            print(json.dumps({"action": "monitor_busy"}))
            return
        ...
        for name, data in cases.items():
            ...
            if before is not None and after is not None:
                data["accepted_gain_since_previous_check"] = after - before
