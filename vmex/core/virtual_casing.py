"""Virtual-casing fields and residuals on a prescribed plasma interface.

This module complements, but does not differentiate, the NESTOR free-boundary
solve in :mod:`vmex.core.freeboundary`. For a boundary prescribed to VMEX, the
plasma-vacuum interface conditions are smooth coil and boundary objectives. At
that interface the total field is tangent,
...
module only (a) adapts a converged/trial ``vmex`` boundary + total field into
the package's :class:`VmecSurfaceFieldData` and (b) wires the resulting plasma
field to an :class:`~vmex.core.mgrid.MgridField` or a plain ``xyz->B``
...
FD-validates to ~1e-9 (see ``tests/test_virtual_casing_physics.py``).
...
try:  # optional dependency (uwplasma/virtual_casing_jax)
    ...
    _HAVE_VCJ = True
except ImportError as _exc:  # pragma: no cover - exercised only when the dep is absent
    _HAVE_VCJ = False
    _IMPORT_ERROR = _exc
...
def surface_field_data_from_wout(
    ...
    *,
    ...
    The wout may be a converged :class:`~vmex.core.wout.WoutData` (from
    :func:`~vmex.core.wout.read_wout` or
    :func:`~vmex.core.wout.wout_from_state` — i.e. a *trial* boundary too).
...
# ---------------------------------------------------------------------------
# Prescribed plasma-vacuum interface
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class PlasmaVacuumInterface:
    """Differentiable virtual-casing residuals on one prescribed interface.
    ...
    @classmethod
    def from_equilibrium(cls, eq, **kwargs) -> "PlasmaVacuumInterface":
        """Build from an :class:`~vmex.core.optimize.Equilibrium` (uses ``eq.wout``)."""
