...
CONSTRAINT_SCALES = np.array([0.002, 0.05, 0.051, 0.11067033897730942])
...
def load_case():
    ...
    if (
        sha256(CASE / "inputs/wout_fixed.nc")
        != json.loads((CASE / "inputs/source_lineage.json").read_text())["wout_sha256"]
    ):
...
def physical_rows(state, rt):
    return jnp.array(
        [
            ...
            statephysics.major_radius(state, rt),
        ]
    )
...
def resolve_targets(initial_physical):
    ...
    return np.array([0.2, 5.0, 5.1, 11.067033897730942])
