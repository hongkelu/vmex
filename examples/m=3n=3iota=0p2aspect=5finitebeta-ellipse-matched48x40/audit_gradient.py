...
def components(u):
    ...
    return 1e6 * jnp.array([normal, 0.25 * pressure, geometry, 1e-10 * jnp.mean(u * u)])


objective = lambda u: jnp.sum(components(u))  # noqa: E731
...
cv = jax.jit(components)
cj = jax.jit(jax.jacrev(components))
x = jnp.asarray(initial_u)
values = np.asarray(cv(x))
jac = np.asarray(cj(x))
...
for label, d in [
    ...
    for eps in (1e-4, 1e-5, 1e-6, 1e-7):
        plus = np.asarray(cv(x + eps * d))
        minus = np.asarray(cv(x - eps * d))
        fd = (plus - minus) / (2 * eps)
        results.append(
            dict(
                direction=label,
                ...
                ad=ad.tolist(),
                fd=fd.tolist(),
                difference=(ad - fd).tolist(),
                forward=((plus - values) / eps).tolist(),
                backward=((values - minus) / eps).tolist(),
            )
        )
summary = dict(
    completed=True,
    components=["normal", "pressure", "geometry", "regularizer"],
