...
print(
    json.dumps(
        dict(
            phase="frozen_nestor_start",
            state_sha256=hashes["initial_fixed_state.npz"],
            coil_sha256=hashes["coils.json"],
        )
    ),
    flush=True,
)
...
metric = pressure_diagnostic(out["delbsq_num"], out["delbsq_den"], 0.01)
assert all(np.array_equal(np.asarray(getattr(state, n)), original[n]) for n in STATE_NAMES)
result = dict(
    completed=True,
    seconds=time.monotonic() - started,
    exact_fixed_state_unchanged=True,
    ordinary_equilibrium_solves=0,
    coil_parameters_zero=True,
    state_sha256=hashes["initial_fixed_state.npz"],
    coil_sha256=hashes["coils.json"],
    phiedge_Wb=float(inp.phiedge),
    resolution=dict(
        ns=int(solver.resolution.ns), ntheta=int(solver.resolution.ntheta), nzeta=int(solver.resolution.nzeta)
    ),
    diagnostic=metric,
