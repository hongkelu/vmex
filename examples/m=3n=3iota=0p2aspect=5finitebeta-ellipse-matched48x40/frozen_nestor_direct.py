...
fit = json.loads((ROOT / "coils_direct/summary.json").read_text())
...
assert inp.lfreeb
raw = json.loads((ROOT / "coils_direct/coils_candidate.json").read_text())
...
summary = dict(
    ...
    coil_sha256=sha(ROOT / "coils_direct/coils_candidate.json"),
    seconds=time.monotonic() - started,
)
(OUT / "summary.json").write_text(json.dumps(summary, indent=2) + "\n")
print(json.dumps(summary), flush=True)
if summary["passed"]:
    (ROOT / "coils_direct/coils_selected.json").write_bytes((ROOT / "coils_direct/coils_candidate.json").read_bytes())
