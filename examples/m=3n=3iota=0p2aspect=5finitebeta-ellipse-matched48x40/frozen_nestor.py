...
sys.path[:0] = [
    str(ROOT.parents[1]),
    str(ROOT),
    "/root/autodl-tmp/vmex-rotating-ellipse-m3n3-maxmode1-full111-20260825T141353Z/external/ESSOS",
]
import jax
...
fit = json.loads((ROOT / "coils/summary.json").read_text())
...
assert inp.lfreeb
raw = json.loads((ROOT / "coils/coils_candidate.json").read_text())
...
summary = dict(
    ...
    coil_sha256=sha(ROOT / "coils/coils_candidate.json"),
    seconds=time.monotonic() - started,
)
(OUT / "summary.json").write_text(json.dumps(summary, indent=2) + "\n")
print(json.dumps(summary), flush=True)
if summary["passed"]:
    (ROOT / "coils/coils_selected.json").write_bytes((ROOT / "coils/coils_candidate.json").read_bytes())
