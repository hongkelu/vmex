...
write(
    ...
    dict(
        ...
        weight=1.0,
        metric="square of the exact reported DEL-BSQ ratio",
        ns=50,
        ntheta=48,
        nzeta=40,
        mf=4,
...
expected_base = prior_summary["objective_final"]
expected_nestor = 1e6 * expected["delbsq"] ** 2
write(
    "objective_consistency.json",
    dict(
        components=start_components.tolist(),
        base_value=float(np.sum(start_components[:-1])),
        expected_base=expected_base,
        nestor_value=float(start_components[-1]),
        expected_nestor=expected_nestor,
    ),
)
...
result = minimize(
    ...
    callback=callback,
    options=dict(maxiter=1000, maxfun=60000, maxls=30, ftol=1e-16, gtol=1e-8, maxcor=30),
)
