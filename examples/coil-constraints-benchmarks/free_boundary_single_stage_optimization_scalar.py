...
def parse_args(argv=None):
    ...
    if args.initial_coils is None and args.coils is None and args.qualification is None:
        args.coils = COILS
...
def qualification_contract(args):
    ...
    return opt.OptimizationQualification.signature(parameters=parameters,
        ...
        sources=[__file__, HERE / "parameters.py", HERE / "_coil_constraints.py", HERE / "_coil_resolution.py"])
...
def verify_endpoint(stage, args, summary, history, initial_equilibrium):
    ...
    try:
        ...
        import _coil_constraints as coil_limits
        surf = SurfaceRZFourier.from_wout_file(wout_path, nphi=coil_limits.VERIFY_SURFACE_GRID[0],
                                              ntheta=coil_limits.VERIFY_SURFACE_GRID[1])
        ...
        print(f"Coil lengths = {lengths} (upper bound {LENGTH_TARGET:.4f} m)")
...
def main(argv=None, *, method="SLSQP"):
    ...
    settings = {name: value for name, value in globals().items() if name.isupper() and name not in ("HERE", "P")}
    if args.dry_run:
        print(json.dumps(dict(optimizer=method, parameters=settings, coil_constraints={k:v for k,v in vars(P).items() if k.isupper()}, arguments=vars(args)), indent=2, default=str))
        return 0
    out = setup_run(args)
    import numpy as np
    from vmex import optimize as opt
    ...
    try:
        ...
        def record_step():
            ...
            iota, radius, clearance_value = physical
            coil_slack = np.asarray(stage.coil_constraint.fun(x)).min()
            row = dict(step=problem.accepted_step, objective=problem.fun(x),
                ...
                optimizer_constraints_feasible=bool(np.min(inequalities(physical)) >= -1e-8 and coil_slack >= -1e-8),
                coil_minimum_scaled_slack=float(coil_slack),coil_surface_distance_m=float(clearance_value),
                constraints_feasible=bool(iota >= IOTA_FLOOR and abs(radius-RADIUS_TARGET) <= RADIUS_TOLERANCE
                    and coil_slack >= 0 and clearance_value >= P.COIL_SURFACE_DISTANCE_LIMIT),
        ...
        try:
            result = run_optimizer(stage, args, record_step, method=method)
