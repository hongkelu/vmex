...
cv = jax.jit(objective_components)
cj = jax.jit(jax.jacrev(objective_components))
jac = np.asarray(cj(x))
checks = []
passed = True
for label, d in [
...
write(
    ...
    dict(
        checks=checks,
        passed=passed,
        criterion="All seven components and their sum agree in current, shape, and mixed directions at two adjacent FD step sizes below 1e-5",
        seed=20260914,
    ),
)
