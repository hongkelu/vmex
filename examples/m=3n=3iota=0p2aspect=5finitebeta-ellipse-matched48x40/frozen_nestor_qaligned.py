...
OUT = ROOT / "frozen_nestor_qaligned"
...
sys.path[:0] = [
    str(ROOT.parents[1]),
    str(ROOT),
    "/root/autodl-tmp/vmex-m3n3-iota02-aspect5-angular48x40-20260914/external/ESSOS",
]
import jax
...
fit = json.loads((ROOT / "coils_qaligned/summary.json").read_text())
...
assert abs(float(np.mean(w.iotas[1:])) - 0.2) <= 1e-5 and abs(float(w.b0) - 5.1) <= 1e-4
...
assert inp.lfreeb
raw = json.loads((ROOT / "coils_qaligned/coils_candidate.json").read_text())
...
with np.load(ROOT / "fixed/state.npz", allow_pickle=False) as d:
    assert bool(d["qualified_fixed_target"])
    original = {n: d[n].copy() for n in names}
...
assert (solver.resolution.ns, solver.resolution.ntheta, solver.resolution.ntheta3, solver.resolution.nzeta) == (
    50,
    48,
    25,
    40,
...
summary = dict(
    ...
    state_sha256=sha(ROOT / "fixed/state.npz"),
    coil_sha256=sha(ROOT / "coils_qaligned/coils_candidate.json"),
    seconds=time.monotonic() - started,
)
(OUT / "summary.json").write_text(json.dumps(summary, indent=2) + "\n")
print(json.dumps(summary), flush=True)
if summary["passed"]:
    (ROOT / "coils_qaligned/coils_selected.json").write_bytes(
        (ROOT / "coils_qaligned/coils_candidate.json").read_bytes()
    )
