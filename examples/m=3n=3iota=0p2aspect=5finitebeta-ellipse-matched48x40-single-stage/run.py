...
def main():
    ...
    parser.add_argument(
        "--initialize-only", action="store_true", help="Certify and export step zero without optimization"
    )
    ...
    signal.setitimer(signal.ITIMER_REAL, args.max_wall_hours * 3600)
    ...
    def checkpoint():
        ...
        data = dict(
            schema_version=np.asarray("vmex.finitebeta-ellipse-four-constraints/v1"),
    ...
    def record_diagnostics():
        ...
        event(
            "plasma_metrics",
            absolute_step=step,
            betatotal=float(volume_average_beta(accepted.state, rt)),
            total_plasma_current_A=float(inp.curtor),
        )
    ...
    try:
        ...
        from initialization import verify_runtime, initial_baselines, ordinary_initial_state  # noqa: F401
        ...
        source = CASE.parents[1]  # noqa: F841
        provenance = dict(
            diagnostics=dict(
            ...
            input_hashes=input_hashes,
            main_commit=json.loads((Path(__file__).resolve().parent / "runtime_manifest.json").read_text())["commit"],
            jax=jax.__version__,
            ...
            vmex_runtime_root=str(actual_runtime),
            source_sha256={
                **{"vmex/" + str(f.relative_to(actual_runtime)): sha256(f) for f in actual_runtime.rglob("*.py")},
                **{
                    "case/" + str(f.relative_to(CASE)): sha256(f)
                    for f in CASE.rglob("*.py")
                    if "runs" not in f.parts and "deployment" not in f.parts
                },
            },
        )
        ...
        event(
            ...
            initial_equilibrium_solves=1 if resume is None else 0,
            resumed=resume is not None,
        )
        from gates import fresh_pressure_gate

        initial_pressure = fresh_pressure_gate(
            solver, params, p, seed, initial_rcon0, initial_zcon0, contract["delbsq_limit"]
        )
        ...
        write(
            ...
            dict(
                ...
                major_radius_m=float(targets[3]),
                start_step=initial_step,
                start_physical=dict(zip(("mean_iota", "aspect_ratio", "b0", "major_radius"), start_physical.tolist())),
                targets_source="explicit user targets: iota=0.2, aspect=5, B0=+5.1 T, Rmajor=11.067033897730942 m"
                if resume is None
                else "authenticated finite-beta checkpoint; targets and normalization preserved",
