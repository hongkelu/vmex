...
CONSTRAINT_SCALES = np.array([.002,.05,.051,.11067033897730942])
ROW_ORDER = ('qs_norm','mean_iota','aspect_ratio','b0','major_radius')
...
class DirectCoilField:
    """Exact JAX-traceable filament field used by the stage-two coil candidate."""
...
def load_case():
    ...
    if (inp.mpol,inp.ntor,inp.nfp,inp.lasym,list(inp.ns_array)) != (3,3,2,False,[50]):
        raise ValueError('resolution/symmetry mismatch')
    if (inp.ntheta,inp.nzeta)!=(contract['ntheta'],contract['nzeta']):
        raise ValueError('angular grid differs from case contract')
    ...
    with np.load(CASE/'inputs/initial_fixed_state.npz',allow_pickle=False) as d:
        data={k:np.asarray(d[k]) for k in d.files}
    if not bool(data['qualified_fixed_target']) or bool(data['eligible_for_free_boundary_resume']):
        raise ValueError('initial state must be a qualified fixed target, not a free-boundary root')
    if sha256(CASE/'inputs/wout_fixed.nc') != json.loads((CASE/'inputs/source_lineage.json').read_text())['wout_sha256']:
        raise ValueError('fixed WOUT lineage mismatch')
    for key in STATE_NAMES:
        if data[key].shape!=(50,18) or data[key].dtype!=np.float64 or not np.all(np.isfinite(data[key])):
            raise ValueError('invalid NS50 fixed seed state')
    source=json.loads((CASE/'inputs/source_lineage.json').read_text())
    if inp.phiedge!=source['phiedge_Wb'] or sha256(CASE/'inputs/coils.json')!=source['coil_sha256']:
        raise ValueError('initial coil/flux lineage mismatch')
    return inp,Full111FieldBuilder(jnp.asarray(curves),jnp.asarray(currents)),SpectralState(*(data[k] for k in STATE_NAMES)),contract,expected



def physical_rows(state,rt):
    return jnp.array([statephysics.mean_iota(state,rt),statephysics.aspect_ratio(state,rt),statephysics.on_axis_magnetic_field(state,rt),statephysics.major_radius(state,rt)])


def resolve_targets(initial_physical):
    values=np.asarray(initial_physical,dtype=float)
    if values.shape!=(4,) or not np.all(np.isfinite(values)) or values[2]==0:
        raise ValueError('finite, nonzero initial signed B0 required')
    return np.array([.2,5.,5.1,11.067033897730942])
...
def make_rows(runtime,targets,loss_scale):
    ...
    # Keep the runtime projection host-eager, matching the verified NESTOR path.
    return rows
...
def metrics(values,targets,loss_scale):
    ...
    return dict(qs_error_raw=float((values[0]*loss_scale)**2),qs_objective_normalized=float(.5*values[0]**2),physical=dict(zip(ROW_ORDER[1:],physical.tolist())),constraint_inf=float(np.max(np.abs(values[1:]))),target_errors=dict(zip(ROW_ORDER[1:],(physical-targets).tolist())))
