...
fixed=json.loads((ROOT/'fixed/summary.json').read_text());fit=json.loads((ROOT/'coils_smooth/summary.json').read_text())
...
assert inp.lfreeb
raw=json.loads((ROOT/'coils_smooth/coils_candidate.json').read_text());builder=Full111FieldBuilder(jnp.asarray(raw['dofs_curves']),jnp.asarray(raw['dofs_currents']));p=jnp.zeros(111)
