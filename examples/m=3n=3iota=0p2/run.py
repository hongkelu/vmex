...
def main():
    ...
    provenance = dict(
        ...
        equilibrium_recompute=False,
        force_tolerance=1e-14,
        projected_root_atol=2e-6,
        maximum_coil_step_m=0.001,
    )
    try:
        ...
        solver = fbi.make_free_boundary_config(
            inp,
            builder(jnp.asarray(checkpoint["parameters"])),
            field_from_parameters=builder,
            device=device,
            ftol=1e-14,
            max_iterations=12000,
            adjoint_solver="reverse_gcrot",
            adjoint_tol=2e-5,
            adjoint_residual_rtol=2e-5,
            adjoint_gcrot_m=30,
            adjoint_gcrot_k=10,
            adjoint_maxiter=100,
            adjoint_fail="error",
            include_edge_in_convergence=True,
            edge_force_tolerance=1e-14,
        )
        ...
        if args.validate_tangent_only:
            solver = dataclasses.replace(
                solver, implicit=dataclasses.replace(solver.implicit, adjoint_tol=2e-6), adjoint_residual_rtol=2e-6
            )
            ...
            for tolerance in (2e-5, 2e-6, 2e-7):
                ...
                trial_solver = dataclasses.replace(
                    solver,
                    implicit=dataclasses.replace(solver.implicit, adjoint_tol=tolerance),
                    adjoint_residual_rtol=tolerance,
                )
                trial_cfg = dataclasses.replace(cfg, solver=trial_solver)
                diagnostics = []
                t = time.monotonic()
                jac = np.asarray(
                    fc.free_boundary_continuation_state_pullback(accepted, trial_cfg, rhs, diagnostics=diagnostics)
                )
            ...
            if max(tight_errors) > 0.001 or motion_errors[1] > 1e-6:
                raise RuntimeError("tighter derivatives/proposal are not stable")
            chosen = 0 if max(errors) <= 0.001 and motion_errors[0] <= 1e-6 else 1
            ...
            event(
                "tolerance_selected",
                rtol=tolerance,
                row_errors=errors.tolist(),
                tight_row_errors=tight_errors.tolist(),
                proposal_difference_m=motion_errors,
            )
            ...
            while step < args.target_step:
                ...
                if count > 64:
                    raise RuntimeError("proposal exceeds continuation budget")
                tangent = fbi.free_boundary_state_tangent(
                    cfg.params,
                    jnp.asarray(accepted.parameters),
                    solver,
                ...
                for index in range(1, count + 1):
                    ...
                    event(
                        ...
                        converged=bool(stage.result.converged),
                    ...
                    previous = fc.certify_free_boundary_continuation_state(
                        cfg, point, stage.result.state, rcon0=stage.rcon0, zcon0=stage.zcon0, result=stage.result
                    )
                    event(
                        "certification",
                        seconds=time.monotonic() - t,
                ...
                if step % args.checkpoint_every == 0:
                    record = save(
                        output, accepted, step, targets, float(checkpoint["data"]["trust_radius"]), provenance
                    )
                    saved_step = step
                event(
                    "promoted",
                    absolute_step=step,
                    segment_step=step - initial_step,
        ...
        summary = dict(
            ...
            final_step=step,
            promoted_steps=0 if step is None else step - initial_step,
