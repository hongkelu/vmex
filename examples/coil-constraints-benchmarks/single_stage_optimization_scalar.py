...
for the shared geometry run separately in ``test_coil_constraints.py``.
Edit shared physical bounds in ``parameters.py``.
"""
...
# Both arms reuse the identical fitted coils by default.
HERE = Path(__file__).resolve().parent
parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("--output", type=Path, default=HERE / "runs" / f"fixed-boundary-{time.time_ns()}")
parser.add_argument("--coils", type=Path, default=HERE / "coils_single_stage_scalar_fitted_1789769333906005000.json", help="reuse fitted coils")
parser.add_argument("--dry-run", action="store_true")
parser.add_argument("--check-gradients", action="store_true", help="qualify objective and all constraints at the seed before optimization")
...
parser.add_argument("--constrained", action="store_true", default=True, help="SLSQP with shared iota and physical-radius inequalities")
args = parser.parse_args()
if args.dry_run:
    print(json.dumps({k:v for k,v in vars(P).items() if k.isupper()}, indent=2))
    raise SystemExit(0)
...
MAX_MODE = P.RESOLUTION[0]
MAXITER = P.ACCEPTED_STEPS
...
if args.constrained:
    METHOD = "SLSQP"
    OPTIONS = {"maxiter": args.maxiter or MAXITER, "ftol": P.OPTIMIZER_FTOL}

ci_smoke = False

DATA = Path(__file__).resolve().parent / "input.rotating_ellipse"
inp = vj.VmecInput.from_file(DATA)
# Same physical seed and M8/N8/NS51 resolution as the free arm.
inp = inp.change_resolution(mpol=P.RESOLUTION[0], ntor=P.RESOLUTION[1], ntheta=P.GRID[0], nzeta=P.GRID[1])
inp = replace(inp, ns_array=np.array([P.RESOLUTION[2]]), ftol_array=np.array([P.EQUILIBRIUM_FTOL]))
if args.constrained and args.ftol is None:
    args.ftol = P.EQUILIBRIUM_FTOL
...
coils0 = resize_coils(Coils.from_json(str(DATA.with_name("coils.initial.scalar.json"))), COIL_ORDER, N_SEGMENTS)
...
coil_terms = [
    ...
    # Coil geometry is enforced by explicit inequalities below. No length equality.
]
...
coil_constraint_values = jax.jit(coil_constraint_values)
coil_constraint_jacobian = jax.jit(jax.jacrev(coil_constraint_values))
...
if args.coils:
    ...
    fitted = resize_coils(Coils.from_json(str(args.coils)), COIL_ORDER, N_SEGMENTS)
...
def record_step(u):
    ...
    coil_rows = np.asarray(coil_constraint_values(jnp.asarray(u)))
    diagnostics["coil_minimum_scaled_slack"] = float(coil_rows.min())
    diagnostics["constraints_feasible"] &= int(coil_rows.min() >= 0)
    diagnostics["optimizer_constraints_feasible"] &= int(coil_rows.min() >= -1e-8)
...
Path("provenance.json").write_text(json.dumps(dict(
    ...
    coil_constraint_settings={k:v for k,v in vars(P).items() if k.isupper()},
    coil_constraint_source_sha256=sha(HERE/"_coil_constraints.py"),
    coil_resolution_source_sha256=sha(HERE/"_coil_resolution.py"),
    constrained=args.constrained, constraint_settings={k:v for k,v in vars(limits).items() if k.isupper() and k != "P"},
...
for filename in ("parameters.py", "_coil_constraints.py", "_coil_resolution.py"):
    Path(filename).write_text((HERE/filename).read_text())
record_step(joint_problem.x0)
derivative_qualified = False
if args.check_gradients:
    # Each FD equilibrium is independently solved from the accepted root;
    # the direct geometry rows move the explicitly varied plasma boundary.
    seed_u = np.asarray(joint_problem.x0)
    direction = np.random.default_rng(16).normal(size=seed_u.size)
    direction /= np.linalg.norm(direction)
    analytic = np.r_[joint_problem.grad(seed_u) @ direction,
                     constraint_jacobian(seed_u) @ direction,
                     np.asarray(coil_constraint_jacobian(seed_u)) @ direction]
    ...
    def independent_rows(u):
        ...
        im._PERTURB_SEED.pop(solver_config, None)
        eq = plasma_problem.equilibrium_from_x(x[:x_boundary0.size])
        value = plasma_loss(eq.state, eq.runtime) + jnp.sum(coil_costs(jnp.asarray(x)))
        return np.r_[float(value), np.asarray(limits.inequalities(limits.physical_values(eq.state, eq.runtime))),
                     np.asarray(coil_constraint_values(jnp.asarray(u)))]

    checks = []
    for h in (1e-3, 3e-4, 1e-4):
        fd = (independent_rows(seed_u+h*direction)-independent_rows(seed_u-h*direction))/(2*h)
        errors = np.abs(fd-analytic)
        passed = bool(np.all(np.isfinite(fd)) and np.all(errors <= 1e-7+1e-3*np.abs(analytic)))
        checks.append(dict(h=h, passed=passed, analytic=analytic.tolist(), finite_difference=fd.tolist(),
                           absolute_errors=errors.tolist()))
        Path("gradient_check.json").write_text(json.dumps(dict(passed=len(checks) == 3 and all(c["passed"] for c in checks[-2:]),
            checks=checks, rtol=1e-3, atol=1e-7, seed=16), indent=2)+"\n")
    if not all(c["passed"] for c in checks[-2:]):
        raise RuntimeError("fixed-boundary objective/constraint derivative qualification failed at refined FD steps")
    derivative_qualified = True
    print("Fixed-boundary objective and all coil/plasma constraints passed independent FD checks.", flush=True)
# Equilibrium derivative qualification is separate from production.
...
result = minimize(joint_problem.fun if args.constrained else joint_problem.value_and_grad, joint_problem.x0,
                  ...
                  constraints=[dict(type="ineq", fun=constraint_values, jac=constraint_jacobian),
                               dict(type="ineq", fun=coil_constraint_values, jac=coil_constraint_jacobian)],
...
final_input = replace(final_input,
    ns_array=np.array([P.VERIFY_NS]),
    ftol_array=np.array([P.VERIFY_FTOL]),
...
surface_final = surfacerzfourier_from_boundary(
    ...
    nphi=coil_limits.VERIFY_SURFACE_GRID[0], ntheta=coil_limits.VERIFY_SURFACE_GRID[1])
...
coil_verification = coil_limits.verify(coils_final, surface_final, "coil_verification.json")
maximum_curvature = max(r["peak_per_m"] for r in coil_verification["coils"])
coil_distance = coil_verification["coil_coil_lower_bound_m"]
coil_surface_distance = coil_verification["refined_coil_surface_distance_m"]
...
unmet = [name for name, passed in coil_verification["checks"].items() if not passed]
...
if normal_field_max > NORMAL_FIELD_LIMIT:
...
Path("optimization_summary.json").write_text(json.dumps(dict(
    ...
    derivative_qualified=derivative_qualified,
    ...
    **limits.diagnostics(final_equilibrium.state, final_equilibrium.runtime),
    final_min_abs_iota=minimum_iota, coil_constraints=coil_verification,
    inequalities_met=not unmet, unmet=unmet), indent=2)+"\n")
if args.no_plots:
    raise SystemExit(0 if not unmet else 1)
