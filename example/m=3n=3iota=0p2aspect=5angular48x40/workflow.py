...
def readiness():
    ...
    if not pin:
        return dict(decision='main_revision_pending',action='none',environment_ready=bool(env),environment_error=error,validation_steps=10,target_step=3000)
    if error and not env:return dict(decision='environment_setup_failed',action='none',error=error)
    if not env:return dict(decision='environment_setup_pending',action='none')
    ...
    c.verify_source()
    if not (ROOT/'registry.json').exists():return dict(decision='validated_initial_checkpoint_pending',action='none')
    return None
def validate_ten(status):
    ...
    for row in history:c.authenticate_checkpoint(row)
    accepted=[]
    for seg in c.read(ROOT/'registry.json')['segments']:
        events=c.lines(ROOT/seg['directory']/'output/progress.jsonl')
        for e in events:
            if e['phase']=='failure' and e.get('error_type')!='TimeoutError':raise AssertionError('numerical failure in validation')
            if e['phase']=='adjoint':
                for row in e['rows']:assert row['accepted'] and row['relative_residual']<=2e-5
            if e['phase']=='promoted':
                assert e['maximum_coil_bound_m']<=.001*(1+1e-12)
                assert e['maximum_current_fraction']<=.01*(1+1e-12)
    ...
    return dict(verified_at_utc=c.now(),accepted_steps=10,checkpoint_sha256=status['metrics']['checkpoint_sha256'],source_manifest_sha256=c.sha(ROOT/'source_manifest.json'),all_gates_passed=True)
def tick(restart=False):
    with (ROOT/'workflow.lock').open('a') as lock:
        try:fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
        except BlockingIOError:return dict(decision='workflow_busy',action='none')
        try:
            waiting=readiness()
            if waiting:result=waiting
            else:
                ...
                if policy['target_step']==10 and result.get('decision')=='complete':
                    ...
                    else:result['validation10_verified']=proof
                result['phase']='validation10' if c.read(ROOT/'policy.json')['target_step']==10 else 'production3000'
        except Exception as exc:
            result=dict(decision='integrity_or_controller_error',action='none',error_type=type(exc).__name__,error=str(exc))
        result['checked_at_utc']=c.now()
        c.write(ROOT/'workflow_status.json',result)
        return result
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('mode',choices=['tick']);p.add_argument('--restart',action='store_true');a=p.parse_args()
