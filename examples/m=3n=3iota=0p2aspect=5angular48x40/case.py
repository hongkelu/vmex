...
def physical_rows(state,rt):
    return jnp.array([statephysics.mean_iota(state,rt),statephysics.aspect_ratio(state,rt),statephysics.on_axis_magnetic_field(state,rt)])
...
def resolve_targets(initial_physical):
    ...
    return np.array([.2,5.,-.17506474574437714])
