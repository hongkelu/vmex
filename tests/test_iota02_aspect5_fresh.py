...
def direction(values, policy=POLICY, jac=None, scales=None):
    return opt.proposal(
        np.asarray(values),
        jacobian() if jac is None else jac,
        np.ones(111) if scales is None else scales,
        TARGETS,
        CS,
        policy,
    )
...
def test_fresh_targets_tolerances_and_signed_b0():
    np.testing.assert_array_equal(case.resolve_targets(np.array([0.1977, 4.7448, TARGETS[2]])), TARGETS)
...
def test_fresh_infeasible_restoration_is_not_amplified():
    ...
    assert opt.motion_bounds(d.delta)[0] <= 0.001 * (1 + 1e-12)
    assert opt.motion_bounds(d.delta)[1] <= 0.01 * (1 + 1e-12)
...
def test_fresh_current_only_direction_has_separate_one_percent_cap():
    ...
    assert opt.motion_bounds(d.delta) == pytest.approx((0, 0.01))
...
def test_fresh_numeric_rejections_exhaust_finite_budget_without_promotion():
    ...
    result = opt.backtrack(np.array([1.0, 0, 0, 0]), d, TARGETS, CS, POLICY, evaluate, records.append)
...
def test_fresh_unexpected_programming_errors_do_not_become_retries():
    ...
    with pytest.raises(TypeError):
        opt.backtrack(np.array([1.0, 0, 0, 0]), direction([1, 0, 0, 0]), TARGETS, CS, POLICY, evaluate, lambda x: None)
...
def test_fresh_small_motion_is_not_convergence():
    ...
    result = opt.backtrack(
        np.array([1.0, 0, 0, 0]),
        d,
        TARGETS,
        CS,
        tiny,
        lambda delta, i: (delta, np.array([1.0, 0, 0, 0])),
        lambda x: None,
    )
...
"""
    env = dict(
        ...
        PYTHONPATH=str(CASE.parents[1]),
    )
    result = subprocess.run(
        [sys.executable, "-B", "-c", code, str(CASE / "run.py"), str(tmp_path)],
        env=env,
        cwd=CASE.parents[1],
        capture_output=True,
        text=True,
