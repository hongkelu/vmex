...
def constraint_state(values, targets, constraint_scales, policy):
    ...
    if (values.shape != (5,) or targets.shape != (4,) or scales.shape != (4,)
...
def proposal(values, jacobian, scales, targets, constraint_scales, policy):
    ...
    Projection uses the four equality Jacobian rows in scaled coordinates.
    ...
    if (scales.shape != (111,) or np.any(scales <= 0)
            or not np.all(np.isfinite(scales)) or jacobian.shape != (5, 111)
